...
const PASS = process.env.NEXT_PUBLIC_ACCESS_PASSWORD

const styles = `
  @import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=DM+Mono:wght@300;400;500&display=swap');
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #0a0a0a; min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: 'DM Mono', monospace; }
  .bg { position: fixed; inset: 0; background: radial-gradient(ellipse 80% 60% at 20% 80%, rgba(255,80,0,0.08) 0%, transparent 60%), radial-gradient(ellipse 60% 50% at 80% 20%, rgba(255,180,0,0.05) 0%, transparent 55%), #0a0a0a; z-index: 0; }
  .card { position: relative; z-index: 2; width: 100%; max-width: 420px; padding: 48px 40px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 2px; animation: fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) both; }
  ...
  .field input { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); border-radius: 2px; padding: 12px 14px; font-family: 'DM Mono', monospace; font-size: 13px; color: #fff; outline: none; transition: border-color 0.2s; }
  ...
  .btn { width: 100%; padding: 13px; background: #ff5000; color: #fff; border: none; border-radius: 2px; font-family: 'DM Mono', monospace; font-size: 12px; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; cursor: pointer; transition: background 0.2s; }
...
export default function LoginPage() {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    if (password === PASS) {
      localStorage.setItem('sbam_auth', 'ok')
      window.location.href = '/'
    } else {
